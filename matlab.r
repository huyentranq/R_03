...
disp('4.a')
syms y(x)
ya = dsolve(diff(y,x) + y == 1, y(0) == 1)
x_tmp=-5:0.1:5;
y_sub= subs(ya,x,x_tmp);
plot(x_tmp,y_sub)
%4b
disp('4.b')
yb=dsolve((x^2+1)*diff(y,x) +3*x*(y-1)==0,y(0)==2)
x_tmp= -5:0.1:5;
y_sub= subs(yb,x_tmp);
plot(x_tmp,y_sub)
%4c
...
disp('4.c')
yc=dsolve(diff(y,x,2)-4*y==exp(x)*cos(x)+x^3,y(0)==1,subs(diff(y,x),x,0)==2)
x_tmp= 5:0.1:10;
y_sub= subs(yc,x_tmp);
plot(x_tmp,y_sub)
